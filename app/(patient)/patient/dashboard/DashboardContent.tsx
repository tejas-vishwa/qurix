// This is an async server component — all DB queries happen here, AFTER the page shell is sent
...
export async function DashboardContent() {
  ...
  // All 3 DB queries run in parallel
  ...
  let patientSummaryText =
    "Based on your recent lab reports from the last 90 days, all your tracked biomarkers are currently within standard reference ranges."
  ...
  latestMetricsMap.forEach((m) => {
    ...
    if (m.refMin !== null && m.refMax !== null) {
      ...
      const criticalThreshold = range * 0.15
  ...
  if (overallStatus === "CRITICAL") {
    patientSummaryText =
      "We have detected critically abnormal values in your recent lab results. Please review the alerts below and seek medical advice immediately."
  } else if (overallStatus === "ATTENTION_NEEDED") {
    patientSummaryText =
      "Some of your recent lab results are slightly out of the standard reference ranges. It is recommended to discuss these with your doctor."
  } else if (latestMetricsMap.size === 0) {
    patientSummaryText =
      "No test results found in the last 90 days. Please upload a recent report to get an automated health summary."
  }

  // ── Render ────────────────────────────────────────────────────────────────
  return (
    <div className="grid gap-6 md:grid-cols-2">
      {/* Action buttons — now inside DashboardContent so they appear with data */}
      <div className="md:col-span-2 flex justify-end gap-2 -mt-[4.5rem] pb-2">
        ...
        <CardContent>
          {overallStatus === "NORMAL" ? (
            <p className="text-sm text-green-700 dark:text-green-400 font-medium">
              All clear! No active alerts in the last 90 days. You are doing great!
            </p>
          ) : (
            <ul className="space-y-4">
              {criticalAlerts.map((alert, i) => (
                <li
                  key={`crit-${i}`}
                  className="bg-red-100 dark:bg-red-900/40 p-3 rounded-md text-sm border border-red-200 dark:border-red-800"
                >
              ...
              {standardIssues.map((alert, i) => (
                <li
                  key={`std-${i}`}
                  className="bg-amber-50 dark:bg-amber-950/30 p-3 rounded-md text-sm border border-amber-100 dark:border-amber-900"
                >
                  <span className="font-semibold text-amber-800 dark:text-amber-400">Attention needed:</span>{" "}
                  {alert.advice} <br />
                  <span className="text-muted-foreground mt-1 block">Result: {alert.result}</span>
                </li>
              ))}
